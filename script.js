...
import tableMatImage from "./tablemat.png";
import bottomCake from "./strawberry2.png";
import middleCake from "./mango.png";
import topcake from "./topcake.png";
...
function createConfetti() {
	...
	var skew = 1;
	var colors = ['#f74d6c', '#ffffff', '#5bf74d'];
	...
	(function frame() {
	  ...
	  var ticks = Math.max(20, 50 * (timeLeft / duration));
	  skew = Math.max(0.8, skew - 0.001);
	
	  confetti({
		particleCount: 2 * ((timeLeft / duration) / 2),
		startVelocity: 0,
		ticks: ticks,
		origin: {
		  x: Math.random(),
...
function createCake() {
  ...
  const textureLoader = new THREE.TextureLoader();
  const cakeTexture = textureLoader.load(bottomCake);
  const midcakeTexture = textureLoader.load(middleCake); 
  const topcakeTexture = textureLoader.load(topcake); 
  ...
  const baseMaterial = new THREE.MeshPhongMaterial({ map: cakeTexture});
  ...
  const middleMaterial = new THREE.MeshPhongMaterial({ map: midcakeTexture });
  ...
  const topMaterial = new THREE.MeshPhongMaterial({ map: topcakeTexture });
  const topMesh = new THREE.Mesh(topGeometry, topMaterial);
...
function extinguishCandle(candle, speed) {
  const flames = candle.children.filter(
    (child) => child.material && child.material.type === "ShaderMaterial"
  );
